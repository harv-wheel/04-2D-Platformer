...
func _physics_process(_delta):
	...
	if should_direction_flip:
		if direction < 0 and not $AnimatedSprite.flip_h:
			...
			$AttackHitbox.cast_to.x = -1*abs($AttackHitbox.cast_to.x)
		if direction > 0 and $AnimatedSprite.flip_h:
			$AnimatedSprite.flip_h = false
			$AnimatedSprite.offset.x = 16
			$AttackHitbox.cast_to.x = abs($AttackHitbox.cast_to.x)
...
func attack():
	if $AttackHitbox.is_colliding():
		var target = $AttackHitbox.get_collider()
		print(target.name)
